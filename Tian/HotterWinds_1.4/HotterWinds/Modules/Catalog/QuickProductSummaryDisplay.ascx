        ...
        <%int count = 0;
          ...
          foreach (PriceMeCommon.Data.ProductCatalog pc in productCatalogArrayF)
          {
              ...
              if (PriceMeCommon.BusinessLogic.CategoryController.IsSearchOnly(pc.CategoryID, PriceMe.WebConfig.CountryId))
              {
                  ...
                  {
                      urltype = 2;
                      url = PriceMe.Utility.GetRootUrl("/ResponseRedirect.aspx?aid=40&pid=" + pc.ProductID + "&rid=" + pc.BestPPCRetailerID + "&rpid=" + pc.BestPPCRetailerProductID + "&countryID=" + PriceMe.WebConfig.CountryId + "&cid=" + pc.CategoryID + "&t=c", PriceMe.WebConfig.CountryId).Replace("&", "&amp;");
