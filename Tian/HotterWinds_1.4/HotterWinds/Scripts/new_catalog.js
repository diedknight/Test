...
(function (_$) {
    ...
    _$.init = function (did, aid, more, less) {
        ...
        //if ($(".catalogSelectionDiv div a").length > 0) {
        //    MoreFilter.show(divID, atagID, _moreText, _lessText, _height);
        //}
    }
...
(function (_$) {
    var DragObj = function () {
        ...
        this.init = function (o) {
            obj = o;
            addEvent(obj, "mousedown", mousedown);
        ...
        var mousedown = function (evt) {
            ...
            while (true) {
                if (o.tagName == "A" || o.tagName == "IMG" || o.id == "dailyDealsProductDesc")
                    return;

                if (o.parentNode && o.parentNode != document.body) {
                    o = o.parentNode;
                } else {
                    break;
                }
            ...
            if (!/(absolute)|(fixed)/i.test(getRuntimeStyle(obj, "position", "absolute")))
                obj.style.position = "absolute";
            startPos = { x: evt.screenX, y: evt.screenY };
        ...
        var mousemove = function (evt) {
            ...
            if (isNaN(left) || left == 0)
                left = parseInt(getRuntimeStyle(obj, "left", 0));
            if (isNaN(top) || top == 0)
                top = parseInt(getRuntimeStyle(obj, "top", 0));

            obj.style.left = left + evt.screenX - startPos.x + "px";
            obj.style.top = top + evt.screenY - startPos.y + "px";
            startPos = { x: evt.screenX, y: evt.screenY };
        }
...
$(document).ready(function () {
    ...
    $(".productInfoDiv_List").each(function () {
    ...
    proccessBestPPCDiv(width);
    //CatalogListResponse(width);
});

function narrowByDivHidden() {
    if ($('#MoreFilterDiv').length > 0)
        $('#narrowByMoreFilterDiv').css('display','none');
    $('h4 .glyphicon-arrow-right').attr("class", "glyphicon glyphicon-arrow-down");
    hidFilterStatus.val('collapse');
}
function narrowByDivShown() {
    $('h4 .glyphicon-arrow-down').attr("class", "glyphicon glyphicon-arrow-right");
    if ($('#MoreFilterDiv').length > 0)
        $('#narrowByMoreFilterDiv').css('display', '');
    hidFilterStatus.val('expand');
}
...
function proccessBestPPCDiv(windowWidth) {
    if (windowWidth < 768) {
        $(".bestPPCDiv a").each(function () {
            $(this).attr("url", $(this).attr("href")).attr("clickme", $(this).attr("onclick"))
                .removeAttr("href").removeAttr("onclick");
        });
    } else {
        $(".bestPPCDiv a").each(function () {
            $(this).attr("href", $(this).attr("url")).attr("onclick", $(this).attr("clickme"))
                .removeAttr("url").removeAttr("clickme");
        });
    }
}

function CatalogShowMore(obj, targetID) {
    var target = document.getElementById(targetID);
    if (target != null) {
        if (target.className.indexOf("show") > -1) {
            target.className = "hide";
            $(obj).html("&nbsp;<span class=\"glyphicon glyphicon-arrow-down\"></span>" + str_More)
                .attr('title', str_More);
        }
        else {
            target.className = "show narrowByRowMore";
            $(obj).html("&nbsp;<span class=\"glyphicon glyphicon-arrow-right\"></span>" + str_Less)
                .attr('title', str_Less);
        }
    }
}

function CatalogListResponse(width) {
    $(".comparePriceDiv").each(function () {
        if (width < 751) {
            var atag = $(this).find(".btnVS a")[0];
            atag.innerHTML = "<span class=\"glyphicon glyphicon-chevron-right\"></span> ";
        }
        else {
            var atag = $(this).find(".btnVS a")[0];
            if (atag != undefined) {
                ...
                else if (atag.className.indexOf("btn-ps") > 0)
                    atag.innerHTML = str_PS;
            }
        }
    });
}
...
function getParameter() {
    var selection = $.getUrlParam('pr');
    if (selection == undefined || selection == null) {
        selection = $.getUrlParam('pcsid');
        if (selection == undefined || selection == null) {
            selection = $.getUrlParam('avs');
            if (selection == undefined || selection == null) {
                selection = $.getUrlParam('samp');
                if (selection == undefined || selection == null) {
                    selection = $.getUrlParam('swi');
                    if (selection == undefined || selection == null) {
                        selection = $.getUrlParam('avsr');
                        if (selection == undefined || selection == null) {
                            selection = $.getUrlParam('m');
                            if (selection == undefined || selection == null) {
                                selection = $.getUrlParam('avg');
                            }
                        }
                    }
                }
...
function DisplayFilterOnReady(width) {

    $('#narrowByDiv').on('hidden.bs.collapse', function () {
        // do something…
        narrowByDivHidden();
    })
    $('#narrowByDiv').on('shown.bs.collapse', function () {
        // do something…
        narrowByDivShown();
    })

    var narrowByDiv = $('#narrowByDiv');
    var cls = narrowByDiv.attr('class');
    var selection = getParameter();
    ...
    var e3 = true;
    if (width < 768) {
        e1 = !(selection == undefined || selection == null);//no selection, not expand
        e2 = (isFromProductPage > 0);//if from product page, and selected manufacture, do not display filter panel
        expand = (e1 || e2);
        if (filterStatusBySorting != '')
            expand = expand && (filterStatusBySorting == 'expand');
    }
    else {
        e1 = true;
        e2 = true;
        e3 = !(filterStatusBySorting == 'collapse');

        expand = (e1 || e2) && e3;
    }

    if (expand) {
        $('#narrowByDiv').collapse('show');
    } else {
        $('#narrowByDiv').collapse('hide');
    }
}
...
function SlideFilterIE() {
    var narrowByDiv = $('#narrowByDiv');
    var narrowByDivObj = document.getElementById('narrowByDiv');
    var myH = narrowByDivObj.style.overflow;

    if (myH == '') {
        narrowByDivObj.style.overflow = 'hidden';
        narrowByDiv.stop().animate({ height: '0px' }, { queue: false, duration: 500, easing: 'swing' });

        var narrowByMoreFilterDivObj = document.getElementById('narrowByMoreFilterDiv');
        narrowByMoreFilterDivObj.style.overflow = 'hidden';
        narrowByMoreFilterDivObj.style.height = 0;
        $('h4 .glyphicon-arrow-right').attr("class", "glyphicon glyphicon-arrow-down");
        hidFilterStatus.val('collapse');

    } else {
        narrowByDivObj.style.overflow = '';
        narrowByDiv.stop().animate({ height: '100%' }, { queue: false, duration: 500, easing: 'swing' });

        if ($('#MoreFilterDiv').length > 0) {
            var narrowByMoreFilterDivObj = document.getElementById('narrowByMoreFilterDiv');
            narrowByMoreFilterDivObj.style.overflow = 'hidden';
            narrowByMoreFilterDivObj.style.height = '30';
            $('#narrowByMoreFilterDiv').height(30);
        }
        $('h4 .glyphicon-arrow-down').attr("class", "glyphicon glyphicon-arrow-right");
        hidFilterStatus.val('expand');
    }
...
function shortenDsc() {
    var html = $(".ctcTitleTd .categoryDec").html().trim();
    if (html.length > 0) {
        $(".ctcTitleTd .categoryDec").css('white-space', 'nowrap').css('text-overflow', 'ellipsis');
        $("#morelessATag").remove();
        $(".ctcTitleTd").append('<a onclick="showDsc();" id="morelessATag">More...</a>');
    }
}

function showDsc() {
    var html = $(".ctcTitleTd .categoryDec").html();
    if (html.length > 0) {
        $(".ctcTitleTd .categoryDec").css('white-space', 'normal').css('text-overflow', 'initial');
        $("#morelessATag").remove();
        $(".ctcTitleTd").append('<a onclick="shortenDsc();" id="morelessATag">Less...</a>');
    }
}

function showDscOnly() {
    if ($(".ctcTitleTd .categoryDec").length > 0) {
        $(".ctcTitleTd .categoryDec").css('white-space', 'normal').css('text-overflow', 'initial');
        $("#morelessATag").remove();
    }
}
...
$(document).ready(function () {
    ...
    setTimeout(initCompare, 310);
});

function initCompare() {
    
    if ($('#compareDiv .container #compareTop3').length > 0) {
        $('#compareDiv .container #compareTop3').css('display', 'block');
    } else {
...
function loadCompareFromCookie() {
    var compareInfos = getCookie('compareInfos');
    var selectedCount = 0;
    if (compareInfos != '') {
        var infos = compareInfos.split('|');
        selectedCount = infos.length;
        for (var i = 0; i < selectedCount; i++) {
            var info = infos[i].split(',', 3);
            $('#imagesList').append('<div id="compareImage_' + info[0] + '" onclick="removeCompareList(' + info[0] + ')"><span class="glyphicon glyphicon-remove"></span><img src="' + info[1] + '" title="' + info[2] + '" width="27px" height="27px"/></div>');
            $('#cb_' + info[0]).prop('checked', true);
            $('#item_' + info[0]).addClass('compareSelected');
        }
    }

    if(selectedCount > 0) {
        $('#compareDiv, #compareDiv #compareSelected').css('display', 'block');
        $('#compareDiv2 #compareTop3').css('display', 'none');

        //$('#compareDiv2').css('display', 'none');
    } else {
        $('#compareDiv, #compareDiv #compareSelected').css('display', 'none');
        $('#compareDiv2 #compareTop3').css('display', 'block');

        //$('#compareDiv2').css('display', 'block');
    }
}

function addCompareToCookie(compareInfo) {
    var compareInfos = getCookie('compareInfos');
    if (compareInfos != '') {
    ...
    setCookie('compareInfos', compareInfos, null);
}

function removeCompareFromCookie(pid) {
    var compareInfos = getCookie('compareInfos');
    if (compareInfos != '') {
        var newCompareInfos = '';
        var infos = compareInfos.split('|');
        for (var i = 0; i < infos.length; i++) {
            var info = infos[i].split(',', 3);
            if ((pid + '') != info[0]) {
                newCompareInfos += infos[i] + "|";
            }
        }
        if (newCompareInfos.length > 1)
        {
            newCompareInfos = newCompareInfos.substr(0, newCompareInfos.length - 1);
        }
        setCookie('compareInfos', newCompareInfos, null);
    }
}

$(window).scroll(function () {
    if ($('#catalogBar').length > 0) {
        var top = $(document).scrollTop();
        var compareDivTop = $('#catalogBar').offset().top + 1;
        if (top > compareDivTop) {
            $('#compareDiv').addClass("fixedDiv");
            $('#compareDiv .compareContainer').addClass('container');
        } else {
            $('#compareDiv').removeClass("fixedDiv");
            $('#compareDiv .compareContainer').removeClass('container');
        }
    }
});

function addToCompareList(pid, alertMSG) {
    var ischecked = $('#cb_' + pid).prop('checked');
    var compareCount = 0;
    var compareInfos = getCookie('compareInfos');
    if (compareInfos != '') {
        var infos = compareInfos.split('|', 3);
        compareCount = infos.length;
    }
    if (ischecked) {
        if (compareCount < 3) {
            $('#item_' + pid).addClass('compareSelected');
            var imgSrc = $('#productImage' + pid).prop('src');
            var alt = $('#productImage' + pid).prop('alt');
            $('#imagesList').append('<div id="compareImage_' + pid + '" onclick="removeCompareList(' + pid + ')"><span class="glyphicon glyphicon-remove"></span><img src="' + imgSrc + '" title="' + alt + '" width="27px" height="27px"/></div>');

            addCompareToCookie(pid + "," + imgSrc + "," + alt);
        } else {
            $('#cb_' + pid).prop('checked', false);
            alert(alertMSG);
        }
        if (compareCount == 0) {
            $('#compareDiv, #compareDiv #compareSelected').css('display', 'block');
        ...
        $('#item_' + pid).removeClass('compareSelected');
        $('#compareImage_' + pid).remove();
        removeCompareFromCookie(pid);
        if (compareCount == 1) {
            $('#compareDiv, #compareDiv #compareSelected').css('display', 'none');
            $('#compareDiv2').css('display', 'block');
            $('#compareDiv2 #compareTop3').css('display', 'block');
...
function removeCompareList(pid) {
    $('#cb_' + pid).prop('checked', false);
    $('#item_' + pid).removeClass('compareSelected');
    $('#compareImage_' + pid).remove();
...
function compareProducts() {
    ...
    //setCookie('compareInfos', '', null);
    //$('.checkDiv input[type="checkbox"]:checked').prop('checked', false);
    location.href = "/Compare.aspx?t=js&pids=" + ids;
...
function compareTop3() {
    ...
    var checkboxs = $('#catalogProductsDiv input:checkbox');
    var maxCount = 3;
...
function clickAttrGroup(groupid) {
    var display = $('#group_' + groupid).css('display');
    if (display == 'block') {
        $('#group_' + groupid).css('display', 'inline-block');
        $('#group_' + groupid + ' .groupValue').css('display', 'none');
        $('#group_' + groupid + ' .groupName > .glyphicon').addClass('glyphicon-triangle-bottom');
        $('#group_' + groupid + ' .groupName > .glyphicon').removeClass('glyphicon-triangle-right');
    } else {
        $('#group_' + groupid).css('display', 'block');
        $('#group_' + groupid + ' .groupValue').css('display', 'block');
        $('#group_' + groupid + ' .groupName > .glyphicon').removeClass('glyphicon-triangle-bottom');
        $('#group_' + groupid + ' .groupName > .glyphicon').addClass('glyphicon-triangle-right');
    }
}
...
function initFilter() {
    ...
    $('#newfilter .nbTitle').click(function () {
        ...
        if (values.hasClass(cn)) {
            ...
            tilte.find('.glyphicon').removeClass('glyphicon-plus').addClass('glyphicon-minus');
        } else {
            values.addClass(cn).hide(time);
            tilte.find('.glyphicon').removeClass('glyphicon-minus').addClass('glyphicon-plus');
        }
    });

    $('#newfilter .groupTitle .glyphicon:not(.checkSpan)').click(function () {
        var tilte = $(this).parent();
        var values = tilte.next();
        if (values.hasClass(cn)) {
            values.removeClass(cn).show(time);
            tilte.find('.glyphicon:not(.checkSpan)').removeClass('glyphicon-chevron-down').addClass('glyphicon-chevron-up');
        } else {
            values.addClass(cn).hide(time);
            tilte.find('.glyphicon:not(.checkSpan)').removeClass('glyphicon-chevron-up').addClass('glyphicon-chevron-down');
        }
    });

    $('#newfilter .groupTitle .glyphicon:not(.checkSpan)').each(function () {
...
function loadFilterProducts() {
    ...
    var avsr = "";
    var os = "";
    $("#newfilter input[type=checkbox]:checked").each(function () {
        var cb = $(this);
        var name = cb.attr('name');
        var value = cb.val();
        if (name == "Attribute") {
            avs += value + '-';
        } else if (name == "AttributeRange") {
            avs += value + 'r-';
        }
    ...
    $("#newfilter input[type=hidden].ppr").each(function () {
        var cb = $(this);
        var name = cb.attr('name');
        var value = cb.val();
    ...
    var swi = $('#newfilter #searchWithIn').val();
    if (swi != "") {
        pprs["swi"] = swi;
    }

    pprs["sb"] = $('#sortBySelect option:selected').val();
    pprs["isAjax"] = "true";

    $.ajax({
        url: '/CatalogAjax.aspx',
        beforsend: run_waitMe('roundBounce'),
        data: pprs,
        dataType : 'html',
        method: 'post',
        success: function (html) {
            var rs = $(html);
            $('#loadDiv').waitMe('hide');
            $('#pcDiv').html(rs.find('#pcDiv').html());
            $('#viewByDiv').html(rs.find('#viewByDiv').html());
            $('#pagerDiv').html(rs.find('#pagerDiv').html());
            $('#sortByDiv').html(rs.find('#sortByDiv').html());
            $('#fCountATag').attr("href", rs.find('#fCountATag').attr("href"));
            
            $('#sortBySelect').change(function () {
                loadFilterProducts();
            });

            $('#filterCategoriesDiv').html(rs.find('#filterCategoriesDiv').html());
            var cn = 'hidded';
            $('#filterCategoriesDiv .moreButton').click(function () {
                var prev = $(this).prev();
                var more = $(this).find('.more')
                if (prev.hasClass(cn)) {
                    prev.removeClass(cn).show();
                    more.html(more.attr('data-less'));
                } else {
                    prev.addClass(cn).hide();
                    more.html(more.attr('data-more'));
                }
            });

            $('#filterCategoriesDiv .moreButton').each(function () {
                if ($(this).prev().find("input[type=checkbox]:checked").length == 0) {
                    $(this).click();
                }
            });

            $('#fCount').html(rs.find('#f_Count').val());
            lazyload({ defObj: "#loadDiv" });

            var cUrl = rs.find('#c_url').val();
            history.pushState(null, null, cUrl);

            var cpCount = rs.find('#c_pCount').val();
            $('#productCountInfo label').html(cpCount);
            if (cpCount == "0") {
                $('#catalogBar').hide(100);
            } else {
                $('#catalogBar').show(100);
            ...
            refreshProductCount(rs, cpCount);
        }
    });
}

function refreshProductCount(rs, cpCount) {
    $('.ajRefreshText').each(function () {
        if (cpCount == "0") {
            $(this).html('(0)');
        } else {
            var id = $(this).attr('id');
...
function attrSlided(titleId, remove, from_value, to_value) {
    var url = $("#url_Raw").val();
    var ar = from_value + "-" + to_value;
    var reg = /(,?)\d+_\d+-\d+/ig;
    var newURL = '';
    var matchedArray = url.match(reg);
    if (matchedArray != null && matchedArray.length > 0) {
        for (var i = 0; i < matchedArray.length; i++) {
            if (matchedArray[i].replace(',', '').indexOf(titleId + "_") > -1) {
                if (remove)
                {
                    newURL = url.replace(matchedArray[i], '');
                } else {
                    if (matchedArray[i].indexOf(',') > -1) {
                        newURL = url.replace(matchedArray[i], ',' + titleId + '_' + ar);
                    } else {
                        newURL = url.replace(matchedArray[i], titleId + '_' + ar);
                    }
                }
        ...
        if (newURL == '') {
            newURL = url.replace(matchedArray[0], matchedArray[0] + ',' + titleId + '_' + ar);
        }
    }

    newURL = newURL.replace('avsr=,', 'avsr=');

    if (newURL == '') {

        if (url.lastIndexOf('.aspx') == url.length - 5) {
            newURL = url + "?avsr=" + titleId + '_' + ar;
        } else {
            newURL = url + "&avsr=" + titleId + '_' + ar;
        }
    }
...
function createFilterSlider(sliderId, valueId, sliderValueType, minValue, maxValue, fromIndex, toIndex, prefixValue, valueFormat) {
    if (valueFormat == 1) {
