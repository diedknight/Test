...
{
    public class NthOfTypeFilterSelector:AbsCompile
    {
        public override bool Vaild(Syntax.AbsSyntax token)
        {
            if (token is ValueFilterSyntax) return ((ValueFilterSyntax)token).FilterName == "nth-of-type";
            if (token is SelectorFilterSyntax) return ((SelectorFilterSyntax)token).FilterName == "nth-of-type";
        ...
        public override HtmlNodeList Run(HtmlNodeList nodeList, Tokens<Syntax.AbsSyntax> tokens)
        {
            ...
            if (tempSyntax is ValueFilterSyntax)
            {
                ...
                return nodeList.Filter(item => item.IndexOfType == value);
            }
            else
            {
                SelectorFilterSyntax syntax = tempSyntax as SelectorFilterSyntax;

                if (syntax.CssSelector == "even") return nodeList.Filter(item => (item.IndexOfType + 1) % 2 == 0);
                if (syntax.CssSelector == "odd") return nodeList.Filter(item => (item.IndexOfType + 1) % 2 != 0);
            }
