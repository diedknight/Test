...
namespace PriceMeCommon.HtmlHelper {
    /// <summary>
    /// UBB 的摘要说明。
    /// </summary>
    public class UBB {
        ...
        public static string UBBToHTML(string sDetail) {
            ...
            r = new Regex(@"(\r\n((&nbsp;)|　)+)(?<ctx>\S+)", RegexOptions.IgnoreCase);
            for (m = r.Match(sDetail); m.Success; m = m.NextMatch()) {
                sDetail = sDetail.Replace(m.Groups[0].ToString(), "<BR>　　" + m.Groups["ctx"].ToString());
            }
            //处理换行，在每个新行的前面添加两个全角空格
            sDetail = sDetail.Replace("\r\n", "<BR>");
            #endregion
            ...
            #endregion
            #region 处[i][/i]标记
            r = new Regex(@"(\[i\])([\s\S]*?)(\[\/i\])", RegexOptions.IgnoreCase);
            for (m = r.Match(sDetail); m.Success; m = m.NextMatch()) {
                sDetail = sDetail.Replace(m.Groups[0].ToString(), "<I>" + m.Groups[2].ToString() + "</I>");
            }
            #endregion
            #region 处[u][/u]标记
            r = new Regex(@"(\[U\])([\s\S]*?)(\[\/U\])", RegexOptions.IgnoreCase);
            for (m = r.Match(sDetail); m.Success; m = m.NextMatch()) {
            ...
            //处[url=xxx][/url]标记
            r = new Regex(@"(\[url=([\s\S]+)\])([ \S\t]*?)(\[\/url\])", RegexOptions.IgnoreCase);
            for (m = r.Match(sDetail); m.Success; m = m.NextMatch()) {
                sDetail = sDetail.Replace(m.Groups[0].ToString(),
                 "<A href=\"" + m.Groups[2].ToString() + "\" rel=\"nofollow\">" +
            ...
            r = new Regex(@"(\[email=([\s\S]+)\])([\s\S]*?)(\[\/email\])", RegexOptions.IgnoreCase);
            for (m = r.Match(sDetail); m.Success; m = m.NextMatch()) {
                sDetail = sDetail.Replace(m.Groups[0].ToString(),
                 "<A href=\"mailto:" + m.Groups[2].ToString() + "\" target=\"_blank\">" +
                 m.Groups[3].ToString() + "</A>");
            ...
            //处[size=x][/size]标记
            r = new Regex(@"(\[size=([1-7])\])([\s\S]*?)(\[\/size\])", RegexOptions.IgnoreCase);
            for (m = r.Match(sDetail); m.Success; m = m.NextMatch()) {
                sDetail = sDetail.Replace(m.Groups[0].ToString(),
            ...
            //处[font=x][/font]标记
            r = new Regex(@"(\[font=([\S]+)\])([\s\S]*?)(\[\/font\])", RegexOptions.IgnoreCase);
            for (m = r.Match(sDetail); m.Success; m = m.NextMatch()) {
                sDetail = sDetail.Replace(m.Groups[0].ToString(),
            ...
            #region 处[IMG][/IMG]标记
            r = new Regex(@"(\[IMG\])(http|https|ftp):\/\/([\s\S]*?)(\[\/IMG\])", RegexOptions.IgnoreCase);
            for (m = r.Match(sDetail); m.Success; m = m.NextMatch()) {
                sDetail = sDetail.Replace(m.Groups[0].ToString(), "<br><a onfocus=this.blur() href=" + m.Groups[2].ToString() + "://" + m.Groups[3].ToString() + " target='_blank' rel='nofollow'><IMG SRC=" + m.Groups[2].ToString() + "://" + m.Groups[3].ToString() + " border=0 alt=\"Open it in new window.\" onload=javascript:if(screen.width-333<this.width)this.width=screen.width-333></a>");
            }
            #endregion
            ...
            r = new Regex(@"\[quote\](?<ctx>[\s\S]*?)\[/quote]", RegexOptions.IgnoreCase);
            for (m = r.Match(sDetail); m.Success; m = m.NextMatch()) {
                sDetail = sDetail.Replace(m.Groups[0].ToString(), "<div class='quote'>" + m.Groups["ctx"].ToString() + "</div>");
            }
            #endregion
