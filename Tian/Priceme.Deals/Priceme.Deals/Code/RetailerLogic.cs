...
{
    private static PriceMeDBDB db = PriceMeCommon.PriceMeStatic.PriceMeDB;
    ...
    public List<RetailerCache> RetailerPageBindRetailerList(List<RetailerCache> rc, List<RetailerCache> rcResult, string sortletter, int rtCatID, Hashtable retailerCategoryHT, out string title, out bool isShowPage)
    {
        ...
        else
        {
            if (rtCatID != 0)
            {
                ...
                Hashtable allActiveRetailersGroupByGategory = RetailerController.AllActiveRetailersGroupByCategory;

                if (allActiveRetailersGroupByGategory.Contains(rcid))
                    rcResult = allActiveRetailersGroupByGategory[rcid] as List<RetailerCache>;
        ...
        {
            isShowPage = true;
            rcResult = RetailerController.RetailerListOrderByClicks;
        }
        ...
        out string title, out bool isShowPage, int pageIndex, int pageSize, out int totalCount)
    {
        ...
        if (!string.IsNullOrEmpty(sortletter))
        {
            if (rtCatID == 0)
            {
                foreach (RetailerCache retailer in RetailerController.retailerListOrderByName)
                {
        ...
        else
        {
            if (rtCatID != 0)
            {
                ...
                string retailerCategoryName = "";
                if (RetailerController.RetailerCategoryHT.Contains(rcid))
                    retailerCategoryName = RetailerController.RetailerCategoryHT[rcid].ToString();
    ...
    public string GetRetailerCategoryName(Hashtable retailerCategoryHT, int rtCatID)
    {
        string retailerCategoryName = "";
        if (rtCatID != 0)
        {
            if (retailerCategoryHT.Contains(rtCatID))
                retailerCategoryName = retailerCategoryHT[rtCatID].ToString();
    ...
    public List<CSK_Store_RetailerPaymentOption> GetRetailerPaymentOption(int retailerId)
    {
        return (from pay in db.CSK_Store_RetailerPaymentOptions where pay.RetailerId == retailerId select pay).ToList();
    }

    public CSK_Store_PaymentOption GetPaymentOption(int paymentOptionId)
    {
