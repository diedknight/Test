    ...
    public static class UrlController
    {
        ...
        public static string GetCategoryBuyingGuidesUrl(Dictionary<string, string> queryParameters)
        {
            if (queryParameters.ContainsKey("bg") )
            {
                ...
                var bg = BuyingGuideController.GetBuyingGuideByBGID(bgid, WebConfig.CountryId);

                string url = "";
                if (bg != null)
                {
                    if (WebConfig.CountryId == 1 || WebConfig.CountryId == 3 || WebConfig.CountryId == 28 || WebConfig.CountryId == 36 || WebConfig.CountryId == 45)
                    {
            ...
            if (queryParameters.ContainsKey("c"))
            {
                ...
                PriceMeCache.CategoryCache cat = CategoryController.GetCategoryByCategoryID(cid, WebConfig.CountryId);
        ...
        private static string BrandUrl(Dictionary<string, string> queryParameters)
        {
            ...
            ManufacturerInfo manu = ManufacturerController.GetManufacturerByID(mid, WebConfig.CountryId);
            if (manu != null)
            {
        ...
        {
            string url = "";
            ManufacturerInfo manu = ManufacturerController.GetManufacturerByID(manufacturerID, WebConfig.CountryId);
            if (manu != null)
            {
        ...
        {
            if (!WebConfig.UrlSeo || WebConfig.CountryId == 41)
        ...
        {
            if(categoryID == 1283 && WebConfig.CountryId == 3)
            {
                return Resources.Resource.Global_HomePageUrl + "/plans/mobile-plans";
            }
            else if(categoryID == 358 && WebConfig.CountryId == 3)
            {
        ...
        {
            RetailerCache retailer = RetailerController.GetRetailerDeep(retailerId, WebConfig.CountryId);
        ...
        private static string GetCatalogPageUrl(string originalUrl)
        {
            ...
            if (match.Success)
            {
                ...
                if (index > 0)
                    newQueryString += ("&" + originalUrl.Substring(index + 1));
                string url = "/newcatalog.aspx?" + newQueryString.Replace("&rid=", "&pcsid=");
        ...
        private static string CatalogUrl(Dictionary<string, string> queryParameters)
        {
            ...
            PriceMeCache.CategoryCache category = CategoryController.GetCategoryByCategoryID(cid, WebConfig.CountryId);
        ...
        private static string GetCatalogUrlDescriptionString(Dictionary<string, string> queryParameters, AttributeParameterCollection urlAttributeParameterList, PriceMeCache.CategoryCache category)
        {
            ...
            if (queryParameters.ContainsKey("m") && queryParameters["m"].Length > 0)
            {
                ...
                ManufacturerInfo manufacturer = ManufacturerController.GetManufacturerByID(mid, WebConfig.CountryId);
            ...
            Dictionary<int, string> selectedAttrRangeValues, List<int> selectedAttributeGroupIDs, string daysRangeString, string onsaleonly)
        {
            ...
            if(selectedAttributeGroupIDs != null && selectedAttributeGroupIDs.Count > 0)
            {
                ...
                {
                    int avgID = PriceMeCommon.BusinessLogic.AttributesController.GetCatalogAttributeGroupID(avID);
        ...
        private static string GetProductUrl(string orgUrl, bool redirectToMobileTouch)
        {
            ...
            if (ma.Success)
            {
                ...
                if (!string.IsNullOrEmpty(PriceMe.WebConfig.ABTestingKey) && orgUrl.Contains("var="))
                {
        ...
        private static string GetRetailerProductUrl(string orgUrl, bool redirectToMobileTouch)
        {
            ...
            if (ma.Success)
            {
                if (redirectToMobileTouch)
                {
                    ...
                    var retailerProduct = ProductController.GetRetailerProduct(rpid, WebConfig.CountryId);
