...
{
    public class ContainsPrefixAttrSelector:AbsCompile
    {
        ...
        public override HtmlNodeList Run(HtmlNodeList nodeList, Tokens<Syntax.AbsSyntax> tokens)
        {
            ...
            tempNodeList.Filter(item => item.GetAttr(syntax.AttrName) == syntax.Value || item.GetAttr(syntax.AttrName).StartsWith(syntax.Value + "-"));
