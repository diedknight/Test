...
{
    public class LastOfTypeFilterSelector:AbsCompile
    {
        public override bool Vaild(Syntax.AbsSyntax token)
        {
            if (!(token is NotValueFilterSyntax)) return false;

            return ((NotValueFilterSyntax)token).FilterName == "last-of-type";
        }
            ...
            return nodeList.Filter(item =>
             {
                 ...
                 return item.ParentNode.ChildNodes.Filter(tempItem => tempItem.NodeName == item.NodeName).Last.IndexOfType == item.IndexOfType;
             });
        }
