...
namespace PriceMeCommon.BusinessLogic
{
    public static class CatalogProductSearchController
    {
            ...
            string onSaleOnly)
        {
            ...
            ProductSearcher productSearcher = new ProductSearcher(keywords, categoryID, manufeaturerIDs, priceRange, selectedAttributeIDs, selectedAttributeRangeIDs, sortBy, selectedRetailers, SearchController.MaxSearchCount_Static, countryId, false, true, false, useSearchonly, daysRange, onSaleOnly, selectedAttrRangeValues);
        ...
        public static void FixAttributesNarrowByInfoProductCount(List<NarrowByInfo> attributesNarrowByInfoList, List<NarrowByInfo> attributesNarrowByInfoListWithP)
        {
            foreach (var ani in attributesNarrowByInfoList)
            {
                ...
                foreach (var aniWithP in attributesNarrowByInfoListWithP)
                {
                    ...
                    {
                        FixFiltersProductCount(ani, aniWithP);
                        break;
                    }
                }
            }
