    ...
    public class ExcelCtrl
    {
        ...
        public ExcelCtrl(int sheetIndex = 0)
        {
            ...
            while (true)
            {
                ...
                data.MaxPrice = row.Count > 9 ? Convert.ToDecimal(row[9]) : 0;
                data.NumberOfPrices = row.Count > 10 ? Convert.ToInt32(row[10]) : 0;
                data.ProductImageUrl = row.Count > 11 ? row[11] : "";
                data.Upcoming = row.Count > 12 ? row[12] : "";
                data.CreatedOn = row.Count > 13 ? row[13] : "";
                data.UpdateOn = row.Count > 14 ? row[14] : "";
                data.ForSale = row.Count > 15 ? row[15] : "";
